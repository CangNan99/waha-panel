...
class QrAndReleaseTests(unittest.TestCase):
    ...
    def test_release_metadata_uses_panel_1_0_4_without_changing_waha_or_network(self):
        ...
        self.assertEqual(release["tag"], "1.0.4")
        self.assertEqual(release["version"], "1.0.4")
        self.assertIn("${PANEL_IMAGE:-docker.io/cangnan88/waha-panel:1.0.4}", compose)
        self.assertIn("${PANEL_VERSION:-1.0.4}", compose)
        self.assertIn('os.environ.get("PANEL_VERSION", "1.0.4")', (root / "panel" / "app.py").read_text(encoding="utf-8"))
        self.assertIn("mem_limit: 256m", compose)
        for installer in (env_example, install_sh, install_ps1):
            self.assertIn("PANEL_IMAGE=docker.io/cangnan88/waha-panel:1.0.4", installer)
            self.assertIn("PANEL_VERSION=1.0.4", installer)
        ...
        self.assertIn("docker.io/cangnan88/waha-panel:1.0.4", install_docs)
        self.assertIn("1.0.2", release_plan)
        self.assertIn("1.0.2", release_design)
        self.assertIn("docker.io/cangnan88/waha-panel:1.0.4", readme)
        self.assertIn("docker.io/cangnan88/waha-panel:1.0.4", readme_zh)
    ...
    def test_update_service_defaults_to_panel_1_0_4(self):
        self.assertEqual(UpdateService().current["panel"], "1.0.4")
