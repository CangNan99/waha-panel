...
<body>
  ...
  <script>
    ...
    async function selectSession(name) { selected = name; $('globalNotice').textContent = ''; $('logs').innerHTML = '<div class="empty">正在读取记录...</div>'; $('qrWrap').innerHTML = qrPlaceholder('idle', '二维码待刷新', '点击“刷新二维码”获取当前会话的登录二维码。'); await refresh(); }
    ...
    async function loadQr() { $('qrWrap').innerHTML = qrPlaceholder('loading', '正在获取二维码', '请稍候，获取完成后会自动显示。'); $('qrButton').disabled = true; try { const response = await fetch(apiSession('/qr?ts=' + Date.now()), {cache:'no-store'}); const type = response.headers.get('content-type') || ''; if (!response.ok || !type.startsWith('image/')) { const data = await response.json(); throw new Error(data.message || '二维码暂不可用'); } const image = document.createElement('img'); image.alt = '当前 WhatsApp 会话二维码'; image.src = URL.createObjectURL(await response.blob()); $('qrWrap').replaceChildren(image); } catch (error) { $('qrWrap').innerHTML = qrPlaceholder('error', '二维码获取失败', `${error.message || '二维码暂不可用'} 请先启动会话，然后点击“刷新二维码”重试。`); } finally { $('qrButton').disabled = false; } }
