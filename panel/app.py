...
class WahaClient:
    ...
    def get_chat_picture(self, session_name, chat_id):
        ...
        return self.get_avatar_bytes(picture_url, max_bytes=MAX_AVATAR_BYTES)
    ...
    def get_avatar_bytes(self, media_url, max_bytes=None):
        target = urljoin(self.base_url + "/", str(media_url or ""))
        parsed = urlparse(target)
        same_origin = self._url_origin(target) == self._url_origin(self.base_url)
        trusted_cdn = (
            parsed.scheme.lower() == "https"
            and parsed.hostname is not None
            and parsed.hostname.lower() in TRUSTED_AVATAR_HOSTS
            and parsed.port in (None, 443)
        )
        if parsed.username or parsed.password or not (same_origin or trusted_cdn):
            raise ValueError("头像地址不属于 WAHA 或受信任的 WhatsApp CDN")
        headers = {
            "Accept": "image/jpeg, image/png, image/webp, application/octet-stream",
        }
        if same_origin:
    ...
    <main id="main">
      ...
      <section class="workspace">
        <article class="panel">
          ...
          <div class="qr-frame" id="qrFrame" data-state="idle" aria-live="polite" aria-busy="false"><div class="legacy-qr-image-layer" id="legacyQrImageLayer"></div><div class="legacy-qr-frosted" id="legacyQrFrosted" data-state="idle" role="status" aria-live="polite"><div class="qr-decoration" aria-hidden="true"><svg viewBox="0 0 100 100" preserveAspectRatio="none" style="display:block;width:100%;height:100%"><filter id="qrGlassNoise"><feTurbulence type="fractalNoise" baseFrequency=".035 .22" numOctaves="4" seed="17" result="noise"/><feColorMatrix in="noise" type="matrix" values=".72 0 0 0 .18 0 .72 0 0 .18 0 0 .72 0 .18 0 0 0 .46 0"/></filter><rect width="100" height="100" fill="#eef0f2"/><rect width="100" height="100" filter="url(#qrGlassNoise)" opacity=".72"/></svg></div><div class="legacy-qr-frosted-copy"><strong id="legacyQrTitle">二维码待刷新</strong><span id="legacyQrDetail">点击刷新二维码</span></div></div></div>
