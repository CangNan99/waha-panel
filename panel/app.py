...
class PanelState:
    def __init__(self, database_path, client, api_key, webhook_secret=None, allowed_cidrs=None,
        ...
        self._update_service = UpdateService(
            current_panel=os.environ.get("PANEL_VERSION", "1.0.6"),
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    @media (max-width:760px) { .shell { padding:20px 14px 36px; } header { display:block; } .header-actions { margin-top:16px; } .db-pill { display:inline-flex; } .metrics, .workspace { grid-template-columns:1fr; } .qr-frame { min-height:260px; } .pairing-form { grid-template-columns:1fr; } }
     ...
     .qr-frame{position:relative;display:block;isolation:isolate;overflow:hidden}.legacy-qr-image-layer,.legacy-qr-frosted{position:absolute;inset:0;display:grid;place-items:center}.legacy-qr-image-layer img{width:100%;height:100%;object-fit:contain;padding:20px;opacity:0;transform:scale(.985);transition:opacity 420ms ease-out,transform 420ms ease-out}.qr-frame[data-state="ready"] .legacy-qr-image-layer img{opacity:1;transform:scale(1)}.legacy-qr-frosted{background:rgba(255,255,255,.72);opacity:.92;transition:opacity 420ms ease-out;backdrop-filter:blur(12px)}.qr-frame[data-state="ready"] .legacy-qr-frosted{opacity:0;pointer-events:none}.legacy-qr-frosted .qr-decoration{width:66%;aspect-ratio:1;background:repeating-linear-gradient(45deg,var(--ink) 0 7px,transparent 7px 17px);filter:blur(10px);opacity:.16}.legacy-qr-frosted-copy{width:min(calc(100% - 34px),260px);padding:16px;border:1px solid color-mix(in srgb,var(--line) 78%,transparent);border-radius:11px;background:rgba(255,255,255,.88);text-align:center;box-shadow:0 8px 24px rgba(31,35,41,.08)}.legacy-qr-frosted-copy strong{display:block;color:var(--ink)}.legacy-qr-frosted-copy span{display:block;margin-top:5px;color:var(--muted);font-size:12px}.qr-frame[data-state="ready"] .qr-placeholder{display:none}.pairing-code{transition:opacity 180ms ease-out,transform 180ms ease-out}.pairing-code[data-state="loading"] .pairing-wait{animation:legacy-pairing-wait 900ms ease-in-out infinite alternate}.pairing-code[data-state="ready"]{animation:legacy-pairing-ready 220ms ease-out both}@keyframes legacy-pairing-wait{from{opacity:.55}to{opacity:1}}@keyframes legacy-pairing-ready{from{opacity:0;transform:translateY(4px)}to{opacity:1;transform:translateY(0)}}@media(prefers-reduced-motion:reduce){.legacy-qr-image-layer img,.legacy-qr-frosted{transition:opacity 160ms ease-out;transform:none}.pairing-code,.pairing-code[data-state]{animation:none;transition:opacity 120ms ease-out;transform:none}}@media(prefers-reduced-transparency:reduce){.legacy-qr-frosted{backdrop-filter:none;background:var(--surface)}}@supports not ((backdrop-filter:blur(1px)) or (-webkit-backdrop-filter:blur(1px))){.legacy-qr-frosted{background:var(--surface)}}
  </style>
</head>
<body>
  <div class="shell">
    ...
    <main id="main">
      ...
      <section class="workspace">
        <article class="panel">
          ...
          <div class="qr-frame" id="qrFrame" data-state="idle" aria-live="polite" aria-busy="false"><div class="legacy-qr-image-layer" id="legacyQrImageLayer"></div><div class="legacy-qr-frosted" id="legacyQrFrosted" data-state="idle" role="status" aria-live="polite"><div class="qr-decoration" aria-hidden="true"></div><div class="legacy-qr-frosted-copy"><strong id="legacyQrTitle">二维码待刷新</strong><span id="legacyQrDetail">点击刷新二维码</span></div></div></div>
          ...
          <div class="pairing">
            ...
            <div class="pairing-code" id="pairingCode" hidden role="status" aria-live="polite" aria-busy="false"></div>
          </div>
  ...
  <script>
    ...
    function setQrState(state, title, detail) { const wrap = $('qrFrame'); wrap.dataset.state = state; wrap.setAttribute('aria-busy', state === 'loading' ? 'true' : 'false'); $('legacyQrFrosted').dataset.state = state; $('legacyQrTitle').textContent = title; $('legacyQrDetail').textContent = detail; if (state !== 'ready') { releaseQrObjectUrl(); $('legacyQrImageLayer').replaceChildren(); } }
    async function revealQrImage(blob, ownsRequest) { const objectUrl = URL.createObjectURL(blob); const image = document.createElement('img'); image.alt = 'WhatsApp 扫码二维码'; image.src = objectUrl; try { await image.decode(); } catch (error) { releaseQrObjectUrl(objectUrl); throw error; } if (!ownsRequest()) { releaseQrObjectUrl(objectUrl); return false; } releaseQrObjectUrl(); qrObjectUrl = objectUrl; $('legacyQrImageLayer').replaceChildren(image); requestAnimationFrame(() => { if (ownsRequest()) setQrState('ready', '二维码已生成', '请使用手机 WhatsApp 扫描。'); }); return true; }
    async function loadQr() { const requestGeneration = ++qrRequestGeneration; const ownsRequest = () => qrRequestGeneration === requestGeneration; setQrState('loading', '正在获取二维码', '请稍候，获取完成后会自动显示。'); $('qrButton').disabled = true; $('notice').textContent = ''; try { const response = await fetch(`/api/qr?ts=${Date.now()}`, {cache:'no-store'}); const type = response.headers.get('content-type') || ''; if (!response.ok || !type.startsWith('image/')) { const data = type.includes('json') ? await response.json() : {}; throw new Error(data.message || '二维码暂不可用'); } await revealQrImage(await response.blob(), ownsRequest); } catch (error) { if (ownsRequest()) { setQrState('error', '二维码获取失败', error.message || '请先创建 / 启动 default 会话。'); $('notice').textContent = error.message; } } finally { if (ownsRequest()) $('qrButton').disabled = false; } }
    ...
    async function requestPairingCode() { const response = await fetch('/api/session/pairing-code', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({phone_number:$('phoneNumber').value})}); const data = await response.json(); if (!response.ok) throw new Error(data.message || '配对码获取失败'); return data; }
    async function pairing() { const generation = ++pairingRequestGeneration; const button = $('pairingButton'); button.disabled = true; button.setAttribute('aria-busy', 'true'); setPairingState('loading'); try { const data = await requestPairingCode(); if (generation !== pairingRequestGeneration) return; setPairingState('ready', data); $('notice').textContent = '配对码已生成，请尽快在手机端输入。'; await refresh(); } catch (error) { if (generation === pairingRequestGeneration) { setPairingState('error', {message:error.message}); $('notice').textContent = error.message; } } finally { if (generation === pairingRequestGeneration) { button.disabled = false; button.setAttribute('aria-busy', 'false'); } } }
    const storedTheme = localStorage.getItem('waha-panel-theme'); if (storedTheme) applyTheme(storedTheme); $('themeSelect').addEventListener('change', (event) => saveTheme(event.target.value)); $('ensureButton').addEventListener('click', ensure); $('qrButton').addEventListener('click', loadQr); $('pairingButton').addEventListener('click', pairing); $('refreshButton').addEventListener('click', refresh); refresh(); setInterval(refresh, 10000);
  </script>
</body>
</html>"""
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>WhatsAPP AI管理面板 · 多会话控制台</title>
  <style>
    :root {
      color-scheme: light;
  ...
  <script>
    ...
    async function refresh() { try { const response = await fetch('/api/status?session=' + encodeURIComponent(selected), {cache:'no-store'}); const data = await response.json(); if (!response.ok) throw new Error(data.message || '读取状态失败'); render(data); } catch (error) { $('globalNotice').textContent = error.message; } }
     ...
     async function revealQrImage(blob, ownsRequest) { const objectUrl = URL.createObjectURL(blob); const image = document.createElement('img'); image.alt = '当前 WhatsApp 会话二维码'; image.src = objectUrl; try { await image.decode(); } catch (error) { releaseQrObjectUrl(objectUrl); throw error; } if (!ownsRequest()) { releaseQrObjectUrl(objectUrl); return false; } releaseQrObjectUrl(); qrObjectUrl = objectUrl; $('qrImageLayer').replaceChildren(image); requestAnimationFrame(() => { if (ownsRequest()) setQrState('ready', '二维码已生成', '请使用手机 WhatsApp 扫描。'); }); return true; }
      function resetQrState() { qrRequestGeneration += 1; pairingRequestGeneration += 1; $('qrButton').disabled = false; releaseQrObjectUrl(); setQrState('idle', '二维码待刷新', '点击“刷新二维码”获取当前会话的登录二维码。'); setPairingState('idle'); }
    ...
    async function selectSession(name) { setSelectedSession(name); $('globalNotice').textContent = ''; $('logs').innerHTML = '<div class="empty">正在读取记录...</div>'; await refresh(); }
     ...
     async function pairing() { const generation = ++pairingRequestGeneration; const button = $('pairingButton'); button.disabled = true; button.setAttribute('aria-busy', 'true'); setPairingState('loading'); try { const data = await requestPairingCode(); if (generation !== pairingRequestGeneration) return; setPairingState('ready', data); $('controlNotice').className = 'notice success'; $('controlNotice').textContent = '配对码已生成，请尽快在手机端输入。'; } catch (error) { if (generation === pairingRequestGeneration) { setPairingState('error', {message:error.message}); $('controlNotice').className = 'notice'; $('controlNotice').textContent = error.message; } } finally { if (generation === pairingRequestGeneration) { button.disabled = false; button.setAttribute('aria-busy', 'false'); } } }
  ...
  <style>
    ...
    * { box-sizing:border-box; } body { margin:0; min-width:320px; background:var(--paper); color:var(--ink); font:15px/1.5 "Segoe UI", system-ui, sans-serif; }
    ...
    label { display:block; font-weight:700; margin:14px 0 6px; } input[type=text], input[type=password], input[type=url], input[type=tel], input[type=time], textarea { width:100%; border:1px solid var(--line); border-radius:6px; padding:10px 11px; color:var(--ink); background:var(--surface); font:inherit; } textarea { min-height:110px; resize:vertical; } input:focus, textarea:focus, select:focus { outline:3px solid var(--blue-bg); border-color:var(--blue); } .check-label { display:flex; align-items:center; gap:10px; min-height:44px; margin:0; } input[type=checkbox] { width:18px; height:18px; accent-color:var(--blue); }
     .schedule { display:grid; gap:8px; align-items:center; } .day-row { display:grid; grid-template-columns:68px 34px minmax(0,1fr) minmax(0,1fr); gap:8px; align-items:center; } .day-row label { margin:0; font-weight:600; } .day-row input[type=time]:disabled { opacity:.55; background:var(--surface-2); } .switch { display:flex; justify-content:space-between; align-items:center; border:1px solid var(--line); border-radius:7px; padding:10px 12px; background:var(--surface-2); } .switch + .switch { margin-top:8px; } .rule-note { margin:8px 0 0; color:var(--muted); font-size:13px; } .range-field { margin-top:20px; padding-top:18px; border-top:1px solid var(--line); } .range-field label { display:flex; justify-content:space-between; align-items:baseline; gap:12px; margin:0 0 6px; } .range-field output { color:var(--blue); font-size:18px; font-weight:800; } .range-field input[type=range] { width:100%; min-height:44px; accent-color:var(--blue); } .media-options { display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:8px; margin:18px 0 0; padding:14px; border:1px solid var(--line); border-radius:7px; } .media-options legend { padding:0 5px; font-weight:700; } .media-options .section-note { grid-column:1/-1; } .media-options label { display:flex; align-items:center; gap:9px; min-height:44px; margin:0; } .media-options input[type=checkbox] { flex:0 0 auto; }
